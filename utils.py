...
class DAVIS(data.Dataset):
    '''
    Dataset for DAVIS
    root is the path of the DAVIS
    '''

    def __init__(self, root, imset='2016/val.txt', resolution='480p', multi_object=False):
        self.root = root
        self.mask_dir = os.path.join(root, 'Annotations', resolution)  # Annotation path
        self.image_dir = os.path.join(root, 'JPEGImages', resolution)  # Image path
        _imset_dir = os.path.join(root, 'ImageSets')  # ImageSets path

        # The file in detail which has the path to the pair of image and annotation in pair
        ...
        self.num_objects = {}
        self.shape = {}
        videoList = os.listdir(self.image_dir)
        print(videoList)
        for line in videoList:
            if (line != ".ipynb_checkpoints"):
                _video = line.rstrip('\n')  # 去掉换行符
                self.videos.append(_video)

                # glob.glob Get all the images in the folder
