...
if MO:
    Testset = DAVIS(DAVIS_ROOT, imset='1080p/val.txt', multi_object=True)
    Testloader = data.DataLoader(Testset, batch_size=1, shuffle=False, num_workers=2, pin_memory=True)
else:
    Testset = DAVIS(DAVIS_ROOT, imset='480p/val.txt')
    Testloader = data.DataLoader(Testset, batch_size=1, shuffle=False, num_workers=2, pin_memory=True)
...
for seq, (all_F, all_M, info) in enumerate(Testloader):
    ...
    for f in range(num_frames):
        ...
        cv2.imwrite(os.path.join(test_path, '{:05d}.png'.format(f)), E)
        """h, w, c = E.shape
        for h0 in range(0,h):
          for w0 in range(0,w):
            for c0 in range(0,c):
        ...
        #cv2.imwrite(os.path.join(test_path, '{:05d}.png'.format(f)), E)
        img_E = Image.fromarray(E).convert('P')
        img_E.load()
